...
surroundWithZeroOrMore :: MonadGen m => Char -> String -> m String
surroundWithZeroOrMore c s = do
...
atom _ = error "invalid atom format"
  ...
  describe "parseSExpr" $ do
    context "property based..." $ do
      it "parses single atoms" $
        require $
        property $ do
          atomStr <- forAll genAtom
          input <- forAll $ surroundWithZeroOrMore ' ' atomStr
          runParser parseSExpr input === Just ((A . atom) atomStr, "")
      it "parses single level of atoms" $
        require $
        property $ do
          atoms <- forAll $ Gen.list (Range.linear 1 20) genAtom
          let input = "(" <> unwords atoms <> ")"
          let expected = Comb (A . atom <$> atoms)
